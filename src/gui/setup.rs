...
pub fn view(app: &RoseApp) -> Element<Message> {
    let missing_source = app.source_input.trim().is_empty();
    let missing_target = app.target_input.trim().is_empty();
    let missing_method = !app.use_api && !app.use_align;

    let is_valid = !missing_source && !missing_target && !missing_method;
